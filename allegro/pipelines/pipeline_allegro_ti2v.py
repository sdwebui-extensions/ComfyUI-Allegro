...
class AllegroTI2VPipeline(DiffusionPipeline):
    ...
    def check_inputs(
        ...
        if mask is None or masked_video is None:
            if conditional_images is None or len(conditional_images) == 0 or conditional_images_indices is None or len(conditional_images_indices) == 0:
                raise ValueError("Must provide either `conditional_images` and `conditional_images_indices` at the same time, or `mask` and `masked_video` at the same time.")
    ...
    def __call__(
        ...
        masked_video: Optional[torch.FloatTensor] = None,
        device: Optional[torch.device] = None,
        ...
        for i, t in progress_wrap(list(enumerate(timesteps))):
            if masked_video is not None and mask is not None: #conditional_images is not None:
    ...
    def prepare_mask_masked_video(
        ...
        masked_video = self.vae.encode(masked_video.to(device), local_batch_size=local_batch_size, callback=callback).latent_dist.sample(generator=generator).mul_(self.vae.scale_factor)
